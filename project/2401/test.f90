...
INTEGER :: numz, numy, maxiter, iter, jmc, sa, sb, s0, alpha, beta
...
REAL(8), DIMENSION(nrepli, 2) :: chi_sq
...
DO jmc = 1, nrepli
    ...
    chi_sq(jmc, 1) = delta * delta / var_delta
    ...
    delta = com_delta(obsy1, obsy2, numz, sa, sb, s0)
    var_delta = com_var_delta (obsy1, obsy2, numz, sa, sb, s0, nbootstrap)
    chi_sq(jmc, 2) = delta * delta / var_delta
    
    IF (chi_sq(jmc, 1) > q_chi_sq) alpha = alpha + 1
    IF (chi_sq(jmc, 2) > q_chi_sq) beta = beta + 1
END DO

    
WRITE(*, *) 'alpha is ', alpha * 1.0 / nrepli
WRITE(*, *) 'beta is ', beta * 1.0 / nrepli

OPEN (20, FILE = 'chi_sq.txt', STATUS = 'REPLACE')
CALL wrtm(chi_sq, 20)
CLOSE(20)
