...
/** @type {T.Mesh} */ const cone = new T.Mesh(coneGeometry, lightMaterial);
  ...
  stepWorld(delta = 0, timeOfDay = 0) {
    ...
    if (delta >= 0 && timeOfDay >= 6 && timeOfDay <= 19) this.light.visible = false;
    else this.light.visible = true;
  }
}

/** @type {number} */ let signCount = 0;
/** @type {T.TextureLoader} */ const textureLoader = new T.TextureLoader();
/** @type {T.Texture} */ const signTexture = textureLoader.load(imageFolder + "stop.png");
/** @type {T.MeshPhongMaterial} */ const whiteMaterial = new T.MeshPhongMaterial({ color: "white" });
/** @type {T.MeshPhongMaterial} */ const redMaterial = new T.MeshPhongMaterial({ color: "red" });
/** @type {T.MeshPhongMaterial} */ const signMaterial = new T.MeshPhongMaterial({ map: signTexture });
...
/** @type {T.Mesh} */ const sign = new T.Mesh(cylinderGeometry, [redMaterial, signMaterial, redMaterial]);
    // Set the transformations for the stem
...
/** @type {number} */ const spin = Math.floor(Math.random() * 4) * 0.5 * Math.PI;
    if (ht == 0) house = new GrSimpleHouse1({ length: 40, width: 50, height: 30, x: 50, z: 60, rotate: spin, index: hc });
    if (ht == 1) house = new GrSimpleHouse2({ length: 60, width: 40, height: 30, x: 50, z: 60, rotate: spin, index: hc });
    if (ht == 2) house = new GrSimpleHouse3({ length: 40, width: 40, height: 30, x: 50, z: 60, rotate: spin, index: hc });
    // Put everything into the group and transform the group
...
/** @type {number} */ const nh = params.nh || 1; // Number of houses
    // Put everything into the group and transform the group
    /** @type {GrStreetLight[]} */ const lights = [];
    for (let i = 0; i < nh; i++) {
...
/** @type {GrSign} */ const stop2 = new GrSign({ radius: 2, thickness: 0.1, x: 2, z: -17, rotate: Math.PI * 0.5 });
  ...
  /**
     ...
     */
  stepWorld(delta = 0, timeOfDay = 0) {
    // Copy the animation from from the lights
    this.lights.forEach(light => light.stepWorld(delta, timeOfDay));
  }
}